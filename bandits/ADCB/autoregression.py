...
def gen_potential_Outcomes(
    ...
    for a in range(8):
        if a == observed_a:
            ...
            if config.latent_dim == "continuous":
                delta, _ = tr.assign_treatment_effect_continuous_no_noise(ABetaRatio, a)
            elif config.latent_dim == 2:
                delta = tr.assign_treatment_effect(a, Z, gamma_0, gamma_1, P1)
            else:
                delta = tr.assign_treatment_effect_n_dim(gamma_0, a, Z)
...
def sample_normal_noise(ADNI_bl, col, n):
    loc, scale = norm.fit(ADNI_bl[col].values)
    rv = norm()
    mean = (rv.mean() * scale) + loc
    noise = ((rv.rvs(size=n) * scale) + loc) - mean
    return noise
...
def fit_auto_regressor(
    ...
    for month in [12, 24, 36, 48]:
        ...
        df1 = df[
            (df["RID"].isin(autoreg_month_values))
            & (df["VISCODE"] == autoreg_steps[month][0])
        ][["RID", Y_col]]
        df1 = df1.rename({Y_col: Y_col + "_prev"}, axis=1)
        df2 = df[
            (df["RID"].isin(autoreg_month_values))
            & (df["VISCODE"] == autoreg_steps[month][1])
        ][cols]
    ...
    data_X = dm.check_categorical(
        data_X, list(data_X.columns), "_", categorical_cols=config.Categorical_cols
    )
        ...
        if grid_search:
            ...
            for k, clf in clf_list.items():
                ...
                lr_results = {
                    ...
                    "acc_tr": acc_tr,
                    "acc_te": acc_te,
                    "acc_std_tr": acc_std_tr,
                    "acc_std_te": acc_std_te,
                    "bacc_tr": bacc_tr,
                    "bacc_te": bacc_te,
                    "f1_tr": f1_tr,
                    "f1_te": f1_te,
                    "n_classes": n_classes,
                    "n_train": data_Xtrain.shape[0],
            ...
            with open(
                config.data_path + "models/" + Y_col + "_autoreg_model.pkl", "wb"
            ) as f:
                pickle.dump(lr, f)
            if not return_metrics:
                return lr
        ...
        if grid_search:
            ...
            for k, reg in reg_list.items():
                ...
                lr_results = {
                    ...
                    "r2_te": r2_te,
                    "y_std": y_std,
                    "res_std_tr": res_std_tr,
                    "res_std_te": res_std_te,
                    "n_train": data_Xtrain.shape[0],
                    "n_test": data_Xtest.shape[0],
                }
...
def generate_N_Samples_bl(
    ...
    if return_metrics:
        ...
        resClassifiers.to_csv(
            "data/resClassifiers_bl.csv", encoding="utf-8", index=False
        )
        resRegressors.to_csv("data/resRegressors_bl.csv", encoding="utf-8", index=False)
        baseline_results = [resClassifiers, resRegressors]

    # load gmm model for sampling Z
    with open(config.data_path + "fitted_gmm_Z.pickle", "rb") as f:
    ...
    for i in range(N):
        ...
        ABetaRatioEpsilon = np.random.normal(loc=ABETARatio, scale=config.z_noise)

        Z = Z_samples[i]  # int(np.random.binomial(size=1, n=1, p=P1))

        # PTETHCAT = choice(list(PTETHCAT_probs.PTETHCAT), 1, p=list(PTETHCAT_probs.prob))
        # PTRACCAT = choice(list(PTRACCAT_probs.PTRACCAT), 1, p=list(PTRACCAT_probs.prob))

        # print("PTETHCAT_probs.PTETHCAT", PTETHCAT_probs.PTETHCAT)
        # print("PTETHCAT_probs.prob", PTETHCAT_probs.prob)

        # choose PTETHCAT, PTRACCAT uniformly
        PTETHCAT = choice(list(PTETHCAT_probs.PTETHCAT), 1)
        PTRACCAT = choice(list(PTRACCAT_probs.PTRACCAT), 1)

        PTGENDER = choice(list(PTGENDER_probs.PTGENDER), 1, p=list(PTGENDER_probs.prob))

        ddict = {"PTETHCAT": PTETHCAT, "PTRACCAT": PTRACCAT, "PTGENDER": PTGENDER}

        APOE4 = lrAPOE4.predict_proba(
        ...
        PTEDUCAT = lrEDUCAT.predict_proba(
            dm.check_categorical(
        ...
        PTMARRY = lrMARRY.predict(
            dm.check_categorical(
                pd.DataFrame({"PTGENDER": PTGENDER}),
                config.all_pred_cols["PTMARRY"],
                "PTMARRY",
            )
        )[0]
        # PTMARRY = lrMARRY.predict_proba(
        #     dm.check_categorical(
        #         pd.DataFrame({"PTGENDER": PTGENDER}),
        #         config.all_pred_cols["PTMARRY"],
        #         "PTMARRY",
        ...
        ddict = {
            ...
            "PTRACCAT": PTRACCAT,
            "PTGENDER": PTGENDER,
            "ABETARatio": ABetaRatioEpsilon,
            "APOE4": APOE4,
            "AGE": AGE,
        }
        TAU = lrTAU.predict(
            dm.check_categorical(
                pd.DataFrame(ddict), config.all_pred_cols["TAU"], "TAU"
            )
        )[0]
        # + noises["TAU"][i]
        # )
        while TAU < 0:
            ...
            # + sample_asymetric(residuals["TAU"].values, 1)[0]
            # )  # sample_noise(AB_ratio_bl_df, 'TAU', 1)[0]

        PTAU = lrPTAU.predict(
            dm.check_categorical(
                pd.DataFrame(ddict), config.all_pred_cols["PTAU"], "PTAU"
            )
        )[0]
        # + noises["PTAU"][i]
        # )
        while PTAU < 0:
            ...
            # + sample_asymetric(residuals["PTAU"].values, 1)[0]
            # )  # sample_noise(AB_ratio_bl_df, 'PTAU', 1)[0]
        ...
        FDG = lrFDG.predict(
            dm.check_categorical(
                pd.DataFrame(ddict), config.all_pred_cols["FDG"], "FDG"
            )
        )[0]
        # + noises["FDG"][i]
        # )
        while FDG < 0:
            ...
            # + sample_asymetric(residuals["FDG"].values, 1)[0]
            # )  # sample_noise(AB_ratio_bl_df, 'FDG', 1)[0]

        AV45 = lrAV45.predict(
            dm.check_categorical(
                pd.DataFrame(ddict), config.all_pred_cols["AV45"], "AV45"
            )
        )[0]
        # + noises["AV45"][i]
        # )
        while AV45 < 0:
            ...
            # + sample_asymetric(residuals["AV45"].values, 1)[0]
            # )  # sample_noise(AB_ratio_bl_df, 'AV45', 1)[0]
        ...
        ADAS13 = lrADAS13.predict(
            dm.check_categorical(
                pd.DataFrame(ddict), config.all_pred_cols["ADAS13"], "ADAS13"
            )
        )[0]
        # + noises["ADAS13"][i]
        # )
        while ADAS13 < 0:
            ...
            # + sample_asymetric(residuals["ADAS13"].values, 1)[0]
            # )  # sample_noise(AB_ratio_bl_df, 'ADAS13', 1)[0]
        if ADAS13 > 85:
            ADAS13 = 85
        ...
        MMSE = lrMMSE.predict(
            dm.check_categorical(
                pd.DataFrame(ddict), config.all_pred_cols["MMSE"], "MMSE"
            )
        )[0]
        # + noises["MMSE"][i]
        # )
        while MMSE < 0:
            MMSE = lrADAS13.predict(
                dm.check_categorical(
                    pd.DataFrame(ddict), config.all_pred_cols["MMSE"], "MMSE"
                )
            )[0]
            # + sample_asymetric(residuals["MMSE"].values, 1)[0]
            # )  # sample_noise(AB_ratio_bl_df, 'MMSE', 1)[0]
        if MMSE > 30:
            MMSE = 30

        CDRSB = lrCDRSB.predict(
            dm.check_categorical(
                pd.DataFrame(ddict), config.all_pred_cols["CDRSB"], "CDRSB"
            )
        )[0]
        # + noises["CDRSB"][i]
        # )
        while CDRSB < 0:
            ...
            # + sample_asymetric(residuals["CDRSB"].values, 1)[0]
            # )  # sample_noise(AB_ratio_bl_df, 'CDRSB', 1)[0]
        if CDRSB > 18:
            CDRSB = 18

        A = 0
        if config.latent_dim == "continuous":
            delta, delta_noise = tr.assign_treatment_effect_continuous_no_noise(
                ABETARatio, A, apoe4=APOE4
            )
        elif config.latent_dim == 2:
            delta = tr.assign_treatment_effect(A, Z, gamma_0, gamma_1, P1)
        ...
        Y_0, Y_1, Y_2, Y_3, Y_4, Y_5, Y_6, Y_7 = gen_potential_Outcomes(
            A, Y_hat, ADAS13, Z, gamma_0, gamma_1, P1, ABETARatio, APOE4=APOE4
        )
        ...
        DX = lrDX.predict_proba(
            dm.check_categorical(pd.DataFrame(ddict), config.all_pred_cols["DX"], "DX")
        )
        ...
        ddict = {
            ...
            "ABetaRatioEpsilon": ABetaRatioEpsilon,
            "TAU": TAU,
            "PTAU": PTAU,
            "APOE4": APOE4,
            "FDG": FDG,
...
def generate_N_Samples_bl_n_dim(
    num_repetition,
    N,
    P1,
    PTETHCAT_probs,
    ...
    if return_metrics:
        ...
        resClassifiers.to_csv(
            config.data_path + "data/resClassifiers_bl.csv",
            encoding="utf-8",
            index=False,
        )
        resRegressors.to_csv(
            config.data_path + "data/resRegressors_bl.csv",
            encoding="utf-8",
            index=False,
        )
        baseline_results = [resClassifiers, resRegressors]
    ...
    Z_samples = np.random.choice(6, N, p=P1)
    ...
    for col in noise_cols:
        # sample_noise(AB_ratio_bl_df, col, N)
        noises[col] = sample_normal_noise(AB_ratio_bl_df, col, N)
        print("Noise: ", col, " :", np.std(noises[col]))
    ...
    for i in range(N):
        ...
        while TAU < 0:
            TAU = (
                ...
                + sample_normal_noise(AB_ratio_bl_df, "TAU", 1)[0]
            )  # sample_asymetric(residuals['TAU'].values, 1)[0]
        ...
        while PTAU < 0:
            PTAU = (
                ...
                + sample_normal_noise(AB_ratio_bl_df, "PTAU", 1)[0]
            )  # sample_asymetric(residuals['PTAU'].values, 1)[0]
        ...
        while FDG < 0:
            FDG = (
                ...
                + sample_normal_noise(AB_ratio_bl_df, "FDG", 1)[0]
            )  # sample_asymetric(residuals['FDG'].values, 1)[0]  #
        ...
        while AV45 < 0:
            AV45 = (
                ...
                + sample_normal_noise(AB_ratio_bl_df, "AV45", 1)[0]
            )  # sample_asymetric(residuals['AV45'].values, 1)[0]  #
        ...
        ADAS13 = (
            ...
            + noises["ADAS13"][i]
        )  # np.random.normal(0, config.ADAS_NOISE / 2, size=1)  # +
        while ADAS13 < 0:
            ADAS13 = (
                lrADAS13.predict(
                    dm.check_categorical(
                ...
                + sample_normal_noise(AB_ratio_bl_df, "ADAS13", 1)[0]
            )  # sample_noise(AB_ratio_bl_df, 'ADAS13', 1)[0]
        if ADAS13 > 85:
            ADAS13 = 85
        ...
        while MMSE < 0:
            MMSE = (
                ...
                + sample_normal_noise(AB_ratio_bl_df, "MMSE", 1)[0]
            )  # sample_noise(AB_ratio_bl_df, 'MMSE', 1)[0]
        if MMSE > 30:
            MMSE = 30
        ...
        while CDRSB < 0:
            CDRSB = (
                ...
                + sample_normal_noise(AB_ratio_bl_df, "CDRSB", 1)[0]
            )  # sample_noise(AB_ratio_bl_df, 'CDRSB', 1)[0]
        if CDRSB > 18:
            CDRSB = 18
        ...
        # 'Y_0': Y_0[0], 'Y_1': Y_1[0], 'Y_2': Y_2[0], 'Y_3': Y_3[0], 'Y_4': Y_4[0], 'Y_5': Y_5[0], 'Y_6': Y_6[0], 'Y_7': Y_7[0]}
        gen_data.append(ddict)
...
def gen_autoregression_samples(
    N,
    num_repetition,
    AB_ratio_bl_df,
    gen_data,
    ...
    if return_metrics:
        ...
        resClassifiers.to_csv(
            "data/resClassifiers_auto_" + str(policy) + ".csv",
            encoding="utf-8",
            index=False,
        )
        resRegressors.to_csv(
            "data/resRegressors_auto_" + str(policy) + ".csv",
            encoding="utf-8",
            index=False,
        )
    ...
    for month in range(12, last_month, 12):
        ...
        pred_df = df2.loc[df2["VISCODE"] == (month - 12)][
        ...
        pred_df = df2.loc[df2["VISCODE"] == (month - 12)][["PTMARRY", "PTGENDER"]]
        PTMARRY = lrMARRY_auto.predict_proba(
            dm.check_categorical(pred_df, list(pred_df.columns), "_")
        )
        PTMARRY = [int(choice(len(x), 1, p=x)) for x in PTMARRY]

        df_past = df2.loc[df2["VISCODE"] == (month - 12)][
            ["TAU", "PTETHCAT", "PTRACCAT", "PTGENDER", "Z"]
        ]
        df_past["APOE4"] = APOE4
        df_past["AGE"] = AGE
        ...
        TAU = (
            lrTAU_auto.predict(
                dm.check_categorical(df_past, list(df_past.columns), "_")
            )
            + noises["TAU"]
        )
        ...
        df_past = df2.loc[df2["VISCODE"] == (month - 12)][
            ["PTAU", "PTETHCAT", "PTRACCAT", "PTGENDER", "Z"]
        ]
        df_past["APOE4"] = APOE4
        df_past["AGE"] = AGE
        df_past["ABETARatio"] = ABETARatio
        ...
        df_past = df2.loc[df2["VISCODE"] == (month - 12)][
            ["ADAS13", "PTETHCAT", "PTRACCAT", "PTEDUCAT", "PTGENDER", "Z"]
        ]
        ...
        df_past = df2.loc[df2["VISCODE"] == (month - 12)][
            ["MMSE", "PTETHCAT", "PTRACCAT", "PTEDUCAT", "PTGENDER", "Z"]
        ]
        df_past["PTMARRY"] = PTMARRY
        df_past["TAU"] = TAU
        df_past["PTAU"] = PTAU
        ...
        df_past = df2.loc[df2["VISCODE"] == (month - 12)][
            ["CDRSB", "PTETHCAT", "PTRACCAT", "PTEDUCAT", "PTGENDER", "Z"]
        ]
        df_past["PTMARRY"] = PTMARRY
        df_past["TAU"] = TAU
        df_past["PTAU"] = PTAU
        ...
        gen_data_auto["MMSE"] = gen_data_auto["MMSE"].map(lambda x: dm.MMSE_cleanup(x))
        ...
        standard_df = pd.DataFrame(
            standard_df, columns=["AGE", "PTEDUCAT", "MMSE", "CDRSB", "ADAS13"]
        )

        gen_data_auto_copy[["AGE", "PTEDUCAT", "MMSE", "CDRSB", "ADAS13"]] = (
            standard_df[["AGE", "PTEDUCAT", "MMSE", "CDRSB", "ADAS13"]]
        )
        if policy == "DX_Based":
            gen_data_auto["A"] = [
                tr.assign_treatment_DX(x, epsilon, policy) for x in prev_dx
            ]
        else:
            # assign_treatment(DX, AGE, GENDER, MARRIED, EDUCATION, MMSE, CDRSB, prev_A, epsilon, policy)
            gen_data_auto["A"] = gen_data_auto_copy.apply(
                lambda x: tr.assign_treatment_Santiago(
                    RACE=x["RACE"],
                    AGE=x["AGE"],
                    GENDER=x["PTGENDER"],
                    MARRIED=x["PTMARRY"],
                    EDUCATION=x["PTEDUCAT"],
                    MMSE=x["MMSE"],
                    CDRSB=x["CDRSB"],
                    prev_A=x["prev_A"],
                    epsilon=epsilon,
        ...
        gen_data_auto["Delta"] = gen_data_auto.apply(
            lambda x: tr.assign_treatment_effect(
                a=x["A"], Z=x["Z"], gamma_0=gamma_0, gamma_1=gamma_1, P_1=P1
            ),
            axis=1,
        )
        ...
        YY = pd.DataFrame(
            np.array(
                list(
                    gen_data_auto.apply(
                        lambda x: gen_potential_Outcomes(
                            observed_a=x["A"],
                            Y_hat=x["Y_hat"],
                            ADAS13=x["ADAS13"],
                            Z=x["Z"],
        ...
        dx_df = df2.loc[df2["VISCODE"] == (month - 12)][
            [
                "DX",
                "PTETHCAT",
                "PTRACCAT",
...
def gen_no_autoregression_samples_continuousZ(
    N,
    num_repetition,
    gen_data_bl,
    ADNIDGPData,
    ...
    noises = {}
    noise_cols = ["TAU", "PTAU", "FDG", "AV45", "MMSE", "ADAS13", "CDRSB"]
    # ADNIDGPData_bl["ABetaRatioEpsilon"] = ADNIDGPData_bl["ABETARatio"]

    lrAPOE4, resAPOE4 = dm.fit_X1_model(
        ADNIDGPData_bl, "APOE4", "CLASSIFICATION", test_size=0.2, return_metrics=True
    )
    lrEDUCAT, resEDUCAT = dm.fit_X1_model(
        ADNIDGPData_bl, "PTEDUCAT", "CLASSIFICATION", test_size=0.2, return_metrics=True
    )
    lrMARRY, resMARRY = dm.fit_X1_model(
        ADNIDGPData_bl, "PTMARRY", "CLASSIFICATION", test_size=0.2, return_metrics=True
    )
    lrTAU, TAURMSE, residuals["TAU"], resTAU = dm.fit_X1_model(
        ADNIDGPData_bl, "TAU", "REGRESSION", test_size=0.2, return_metrics=True
    )
    lrPTAU, PTAURMSE, residuals["PTAU"], resPTAU = dm.fit_X1_model(
        ADNIDGPData_bl, "PTAU", "REGRESSION", test_size=0.2, return_metrics=True
    )
    lrFDG, FDGRMSE, residuals["FDG"], resFDG = dm.fit_X1_model(
        ADNIDGPData_bl, "FDG", "REGRESSION", test_size=0.2, return_metrics=True
    )
    lrAV45, AV45RMSE, residuals["AV45"], resAV45 = dm.fit_X1_model(
        ADNIDGPData_bl, "AV45", "REGRESSION", test_size=0.2, return_metrics=True
    )
    lrADAS13, ADAS13RMSE, residuals["ADAS13"], resADAS13 = dm.fit_X1_model(
        ADNIDGPData_bl, "ADAS13", "REGRESSION", test_size=0.2, return_metrics=True
    )
    lrMMSE, MMSERMSE, residuals["MMSE"], resMMSE = dm.fit_X1_model(
        ADNIDGPData_bl, "MMSE", "REGRESSION", test_size=0.2, return_metrics=True
    )
    lrCDRSB, CDRSBRMSE, residuals["CDRSB"], resCDRSB = dm.fit_X1_model(
        ADNIDGPData_bl, "CDRSB", "REGRESSION", test_size=0.2, return_metrics=True
    )
    lrDX, resDX = dm.fit_X1_model(
        ADNIDGPData_bl, "DX", "CLASSIFICATION", test_size=0.2, return_metrics=True
    )

    np.random.seed(num_repetition)
    last_month = num_steps  # + 12
    ...
    ddict = {
        ...
        # "TAU": gen_data_bl.TAU,
        # "PTAU": gen_data_bl.PTAU,
        # "FDG": gen_data_bl.FDG,
        # "AV45": gen_data_bl.AV45,
        # "Z": gen_data_bl.Z,
        "ABETARatio": gen_data_bl.ABETARatio,
        # "ADAS13": gen_data_bl.ADAS13,
        # "MMSE": gen_data_bl.MMSE,
        # "CDRSB": gen_data_bl.CDRSB,
    }

    # length_dict = {key: len(value) for key, value in ddict.items()}
    gen_data_auto = pd.DataFrame(ddict)
    # print("gen_data_auto", gen_data_auto.describe())
    for month in range(1, last_month, 1):
        ...
        for col in noise_cols:
            ...
            noises[col] = sample_asymetric(residuals[col].values, N)
        # if month % 12 == 0:
        #     gen_data_auto["AGE"] = gen_data_auto["AGE"].map(lambda x: float(x) + 1)

        gen_data_auto["ABetaRatioEpsilon"] = np.random.normal(
            loc=gen_data_auto["ABETARatio"], scale=config.z_noise
        )

        # pred_df = gen_data_auto[["PTETHCAT", "PTRACCAT", "PTGENDER"]]
        # PTEDUCAT = lrEDUCAT.predict_proba(
        #     dm.check_categorical(
        #         pd.DataFrame(pred_df), config.all_pred_cols["PTEDUCAT"], "PTEDUCAT"
        #     )
        # )
        # PTEDUCAT = [int(choice(len(x), 1, p=x)) for x in PTEDUCAT]
        # gen_data_auto["PTEDUCAT"] = PTEDUCAT

        # pred_df = gen_data_auto[["PTGENDER"]]
        # PTMARRY = lrMARRY.predict_proba(
        #     dm.check_categorical(
        #         pred_df,
        #         config.all_pred_cols["PTMARRY"],
        ...
        ADAS13 = lrADAS13.predict(
            dm.check_categorical(
                pd.DataFrame(pred_df), config.all_pred_cols["ADAS13"], "ADAS13"
            )
        )
        # + noises["ADAS13"]
        # )  # np.random.normal(0, config.ADAS_NOISE / 2, size=1)  # +

        gen_data_auto["ADAS13"] = ADAS13
        ...
        MMSE = lrMMSE.predict(
            dm.check_categorical(
                pd.DataFrame(pred_df), config.all_pred_cols["MMSE"], "MMSE"
            )
        )
        # + noises["MMSE"]
        # )

        gen_data_auto["MMSE"] = MMSE
        pred_df = gen_data_auto[
            [
                "PTETHCAT",
                "PTRACCAT",
        ...
        CDRSB = lrCDRSB.predict(
            dm.check_categorical(
                pd.DataFrame(pred_df), config.all_pred_cols["CDRSB"], "CDRSB"
            )
        )
        # + noises["CDRSB"]
        # )
        gen_data_auto["CDRSB"] = CDRSB
        ...
        # ##print(length_dict)
        prev_dx = list(df2.loc[df2["VISCODE"] == (month - 1)]["DX"])
        prev_A = list(df2.loc[df2["VISCODE"] == (month - 1)]["A"])
        ...
        if policy == "DX_Based":
            ...
            gen_data_auto["A"] = np.random.choice(list(range(8)), N)
        else:
            # assign_treatment(DX, AGE, GENDER, MARRIED, EDUCATION, MMSE, CDRSB, prev_A, epsilon, policy)
            gen_data_auto["A"] = gen_data_auto_copy.apply(
                lambda x: tr.assign_treatment_Santiago(
        ...
        # gen_data_auto["Delta"] = gen_data_auto.apply(
        #     lambda x: tr.assign_treatment_effect_continuous(
        #         a=x["A"], z=x["ABETARatio"], apoe4=x["APOE4"]
        #     ),
        #     axis=1,
        # )
        ...
        #     lambda x: x["ADAS13"] + x["Delta"], axis=1
        # )
        # gen_data_auto[["Delta", "Delta_noise"]] =
        res_ = np.array(
            gen_data_auto.apply(
        ...
        # print(gen_data_auto["Delta"].head())
        # print(gen_data_auto["Delta_noise"].head())
        ...
        gen_data_auto["Z"] = gen_data_auto.apply(lambda x: x["PTGENDER"] * 0, axis=1)
        ...
        dx_df = df2.loc[df2["VISCODE"] == (month - 1)][
            [
                "DX",
                "PTETHCAT",
                "PTRACCAT",
        ...
        dx_df["ADAS13"] = gen_data_auto["Y_hat"].values
        dx_df["ADAS13"] = gen_data_bl["ADAS13"].values
        lrDX_auto, resDX = fit_auto_regressor(
            ADNIDGPData,
            autoreg_key_lists,
            autoreg_steps,
        ...
        # lrDX, resDX = dm.fit_X1_model(
        #     dx_df, "DX", "CLASSIFICATION", test_size=0.2, return_metrics=True
        # )

        # DX = lrDX.predict_proba(dm.check_categorical(dx_df, list(dx_df.columns), "_"))
...
def gen_autoregression_samples_continuousZ(
    N,
    num_repetition,
    AB_ratio_bl_df,
    gen_data,
    ...
    Z = gen_data.Z
    # ABETARatio = gen_data.ABETARatio
    ABETARatio = gen_data.ABETARatio.values + np.random.normal(
        0, config.z_noise / 2, size=1
    )
    # print('\nABETARatio', ABETARatio)
    PTETHCAT = gen_data.PTETHCAT
    PTRACCAT = gen_data.PTRACCAT
    ...
    for month in range(12, last_month, 12):
        ...
        #     lambda x: x["ADAS13"] + x["Delta"], axis=1
        # )
        gen_data_auto["Y_hat"] = gen_data_auto.apply(lambda x: 0 + x["Delta"], axis=1)
...
def gen_autoregression_samples_n_dim(
    N,
    num_repetition,
    AB_ratio_bl_df,
    gen_data,
    ...
    with open(config.ADAS13RMSE_file, "wb") as f:
        pickle.dump(ADAS13RMSE, f)
    ...
    if return_metrics:
        ...
        resClassifiers.to_csv(
            config.data_path + "data/resClassifiers_auto_" + str(policy) + ".csv",
            encoding="utf-8",
            index=False,
        )
        resRegressors.to_csv(
            config.data_path + "data/resRegressors_auto_" + str(policy) + ".csv",
            encoding="utf-8",
            index=False,
        )
        autoreg_results = [resClassifiers, resRegressors]
    ...
    for month in range(12, last_month, 12):
        ...
        for col in noise_cols:
            ...
            if month == (last_month - 12):
                print("Autoreg Noise: ", col, " :", np.std(noises[col]))
        ...
        ADAS13 = (
            ...
            + noises["ADAS13"]
        )  # + np.random.normal(0, config.ADAS_NOISE / 2, size=(len(df2.loc[df2['VISCODE'] == (month - 12)]))).reshape(-1)  #
        ...
        if policy == "DX_Based":
            ...
            # assign_treatment_effect_n_dim(gamma, a, Z,

        gen_data_auto["Delta"] = gen_data_auto.apply(
            lambda x: tr.assign_treatment_effect_n_dim(
                a=x["A"], Z=x["Z"], gamma=gamma_0
            ),
            axis=1,
        )
