...
def assign_treatment_effect_n_dim(
    gamma, a, Z, A_Delta=config.A_Delta, beta=config.beta
):
    delta = A_Delta[int(a)] + gamma * A_Delta[int(a)] * beta[int(a)][int(Z)]
    return delta


def assign_treatment_effect_continuous(
    z, a, apoe4=None, alpha_values=config.alpha_values, gamma_values=config.gamma_values
):
    alpha_i = alpha_values[int(a)]
    gamma_i = gamma_values[int(a)]
    return alpha_i * z + gamma_i, alpha_i * z + gamma_i - np.random.normal(
        loc=0, scale=config.ATE_noise / 2
    )


def assign_treatment_effect_continuous_no_noise(
    z, a, apoe4=None, alpha_values=config.alpha_values, gamma_values=config.gamma_values
):
    alpha_i = alpha_values[int(a)]
    gamma_i = gamma_values[int(a)]
    return alpha_i * z + gamma_i, np.random.normal(loc=0, scale=config.ATE_noise / 2)


def assign_treatment_effect_continuous_apoe4(
    z, a, apoe4=None, alpha_dict=config.alpha_dict, gamma_dict=config.gamma_dict
):
    alpha_i = alpha_dict[int(apoe4)][int(a)]
    gamma_i = gamma_dict[int(apoe4)][int(a)]
    return alpha_i * z + gamma_i - np.random.normal(loc=0, scale=config.ATE_noise / 2)


def gen_beta(K, Z_dim):

    coeffs = [-1, 0, 1]
    beta = []

    for i in range(K):
        sam = np.random.choice(coeffs, Z_dim, p=[1 / 3, 1 / 3, 1 / 3])
        while sum(sam) != 0:
            sam = np.random.choice(coeffs, Z_dim, p=[1 / 3, 1 / 3, 1 / 3])
        beta.append(sam)
        continue
...
def assign_treatment_Santiago(
    DX, RACE, AGE, GENDER, MARRIED, EDUCATION, MMSE, CDRSB, prev_A, epsilon, policy
):
    treatments_by_type = [[0], [1, 2, 3, 4, 5], [6], [7]]
    p = np.random.random()
    c_AchEI, c_Memantine = compute_Cs(
        RACE, AGE, GENDER, MARRIED, EDUCATION, MMSE, CDRSB, prev_A
    )
...
def compute_Cs(RACE, AGE, GENDER, MARRIED, EDUCATION, MMSE, CDRSB, prev_A):
    ...
    logit_AchEI = (
        config.OR_AchEI["intercept"]
        + race_coeff_AchEI
        + gender_coeff_AchEI
        + config.OR_AchEI["age"] * float(AGE)
        + edu_coeff_AchEI
        + married_coeff_AchEI
        + config.OR_AchEI["MMSE"] * float(MMSE)
        + config.OR_AchEI["CDR"] * float(CDRSB)
        + 1
    )

    logit_Memantine = (
        config.OR_Memantine["intercept"]
        + race_coeff_Memantine
        + gender_coeff_Memantine
        + config.OR_Memantine["age"] * float(AGE)
        + edu_coeff_Memantine
        + married_coeff_Memantine
        + config.OR_Memantine["MMSE"] * float(MMSE)
        + config.OR_Memantine["CDR"] * float(CDRSB)
        + (config.OR_Memantine["prev_AchEI"] * bool_prev_AchEI)
        + 1
    )

    # logit_AchEI = config.OR_AchEI['intercept'] + race_coeff_AchEI + config.OR_AchEI['gender']*int(GENDER)+ edu_coeff_AchEI*int(EDUCATION) + married_coeff_AchEI + config.OR_AchEI['MMSE']*float(MMSE)

    # logit_Memantine = config.OR_Memantine['intercept'] + race_coeff_Memantine + config.OR_Memantine['gender']*int(GENDER) + edu_coeff_Memantine*int(EDUCATION) + married_coeff_Memantine + config.OR_Memantine['MMSE']*float(MMSE)+config.OR_Memantine['prev_AchEI']*bool_prev_AchEI

    ##print('logit_AchEI, logit_Memantine : ', logit_AchEI, logit_Memantine)

    p_AchEI = sigmoid(np.log(np.log(logit_AchEI)))  # **1/8)
    p_Memantine = sigmoid(np.log(np.log(logit_Memantine)))  # **1/8)
