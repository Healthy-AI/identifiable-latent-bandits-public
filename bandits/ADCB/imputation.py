...
class Imputation:
    ...
    def fit_impute(self, return_metrics=False):
        ...
        # ADNIDGPData["ABetaRatioEpsilon"] = ADNIDGPData["ABETARatio"]
        # print("ADNIDGPData.columns", ADNIDGPData.head())

        PTETHCAT_probs, PTRACCAT_probs, PTGENDER_probs = (
            data_loader.race_gender_bl_statistics(ADNIDGPData)
        )
        ...
        if config.latent_dim == 2 or config.latent_dim == "continuous":
        ...
        for month in months:
            ...
            if config.latent_dim == 2 or config.latent_dim == "continuous":
                df = (
                    ADNI_DGP_NoNaNs_df.loc[ADNI_DGP_NoNaNs_df["RID"].isin(nan_RIDs)][
                        [
                            "RID",
                            "AGE",
                            "PTGENDER",
        ...
        for month in months:
            df = ADNI_DGP_NoNaNs_df[
                ADNI_DGP_NoNaNs_df["RID"].isin(observed_idxs_dict[month])
            ]
            df = df.loc[df["VISCODE"] == month]
            observed_row_ids_dict[month] = [int(x) for x in list(df.row_id)]

            df = ADNI_DGP_NoNaNs_df[
                ADNI_DGP_NoNaNs_df["RID"].isin(imputed_idxs_dict[month])
            ]
            df = df.loc[df["VISCODE"] == month]
        ...
        models = {
            "TAU": {"model": lrTAU, "RMSE": TAURMSE},
            "PTAU": {"model": lrPTAU, "RMSE": PTAURMSE},
            "FDG": {"model": lrFDG, "RMSE": FDGRMSE},
            "AV45": {"model": lrAV45, "RMSE": AV45RMSE},
            "ADAS13": {"model": lrADAS13, "RMSE": ADAS13RMSE},
            "MMSE": {"model": lrMMSE, "RMSE": MMSERMSE},
            "CDRSB": {"model": lrCDRSB, "RMSE": CDRSBRMSE},
            "DX": {"model": lrDX},
        }
        ...
        autoreg_key_lists_imp = dm.get_autoreg_keys(ADNI_DGP_NoNaNs_df, patient_ids)
