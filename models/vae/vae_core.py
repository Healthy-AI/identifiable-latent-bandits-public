...
"""Variational Auto Encoder (VAE) and beta-VAE.

https://github.com/yzhao062/pyod/blob/master/pyod/models/vae.py
...
from models.vae.base import BaseDeepLearningDetector
from models.vae.utils import pairwise_distances_no_broadcast
from models.torch_utils import LinearBlock, get_criterion_by_name
...
class VAE(BaseDeepLearningDetector):
    ...
    def __init__(self, lr=1e-3, epoch_num=30, batch_size=32,
                 optimizer_name='adam',
                 device=None, random_state=42, preprocess=False,
                 ...
                 batch_norm=False, dropout_rate=0.2, **kwargs):
        super(VAE, self).__init__(lr=lr, epoch_num=epoch_num,
    ...
    def evaluating_forward(self, batch_data):
        ...
        x = x.to(self.device)
        x_recon, z_mu, z_logvar = self.model(x)
        score = pairwise_distances_no_broadcast(x.cpu().numpy(), x_recon.cpu().numpy())
        loss = self.criterion(x, x_recon, z_mu, z_logvar,
                              beta=self.beta, capacity=self.capacity)
        return loss, score, z_mu
