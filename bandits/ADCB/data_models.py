...
def fit_for_NaNs(
    ADNI_DGP_NoNaNs_df, imputed_idxs_dict, month, col, predcols, model, class_or_reg
):
    # print("******\n\n", col, "\n", predcols, model)
    if len(imputed_idxs_dict[month]) > 0:
        if class_or_reg == "REGRESSION":
            ...
            pred = model["model"].predict(df_pred) + np.random.normal(
                0, model["RMSE"], size=(len(df))
            ).reshape(-1)
            pred = [check_range(x) for x in pred]

            # #print(pred)
            # print(len(ADNI_DGP_NoNaNs_df.loc[ADNI_DGP_NoNaNs_df['row_id'].isin(row_ids), col]))
            ADNI_DGP_NoNaNs_df.loc[ADNI_DGP_NoNaNs_df["row_id"].isin(row_ids), col] = (
                pred
            )
...
def standardize_x(df1, cols, Y_col, continuous_cols=config.continuous_cols):
    ...
    standardize_cols = [c for c in cols if (((c in continuous_cols)) and (c != Y_col))]
            ...
            df = pd.concat(
                [
                    ...
                    pd.get_dummies(
                        df[col].astype(
                            pd.CategoricalDtype(
                                categories=list(range(categorical_cols[col]))
                            )
                        ),
                        prefix=col,
                        drop_first=True,
