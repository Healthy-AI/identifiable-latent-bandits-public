...
class LatentStateFit:
    ...
    def fitZ_baseline(self, ADNI_DGP_df, latent_dim=config.latent_dim):
        ...
        means = gmm.means_  # Means of the Gaussian components
        ...
        # # Fit GMMs for each value of APOE4 (0, 1, 2)
        # gmm_models = {}
        # for apoe_value in [0, 1, 2]:
        #     # Filter the data for the current APOE4 value
        #     filtered_df = Z_df[Z_df["APOE4"] == apoe_value]

        #     # Extract the ABETARatio column and reshape it for the GMM
        #     AB_ratio = np.array(filtered_df.ABETARatio).reshape(-1, 1)

        #     # Fit the GMM with 2 components
        #     gmma = GMM(n_components=2, random_state=0).fit(AB_ratio)

        #     # Store the fitted GMM model
        #     gmm_models[apoe_value] = gmma

        # # Now gmm_models contains the GMMs for APOE4 values 0, 1, and 2
        # # You can access the means, covariances, and weights of each model like this:
        # for apoe_value, gmma in gmm_models.items():
        #     print(f"GMM for APOE4 = {apoe_value}")
        #     print("Means:", gmma.means_.flatten())
        #     print("Covariances:", gmma.covariances_.flatten())
        #     print("Weights:", gmma.weights_.flatten())
        #     print()
        ...
        if latent_dim == 6:
            Z_df["Z"] = Z_df.apply(
                lambda x: self.latent_dim_6(z=x["Z"], APOE4=x["APOE4"]), axis=1
            )
    ...
    def get_all_unique_RIDs(self, df):
        ...
        for i in range(0, 132, 12):
            ...
            if i == 0:
                ...
                absent_rids = list(
                    np.setdiff1d(np.array(unique_RIDs), np.array(present_rids))
                )
                ...
                present_rids_df = df[
                    (df["VISCODE"] == i) & (df["RID"].isin(absent_rids))
                ][["VISCODE", "RID", "ABETARatio", "TAU", "APOE4"]]
                ...
                if len(new_present_rids) > 0:
                    new_present_rids_df = df[
                        (df["VISCODE"] == i) & (df["RID"].isin(new_present_rids))
                    ]
                    ...
                    absent_rids = list(set(absent_rids) - set(new_present_rids))
